...
The clamp puts out a tiny current. The burden resistor turns it into a small
voltage, and the chip reads that voltage.

**This block has 10 parts.** Work down the table and you are done. Nothing here
needs to be understood to be drawn correctly.

## Step 1 — create four names

Everything in this block joins one of **four points**. Create these four net
names in your schematic first. After that, every part is just "connect these two
names".

| Net name | What it is |
|---|---|
| **CLAMP IN** | the clamp's white wire, before the fuse |
| **CLAMP+** | after the fuse — the top of the burden resistor |
| **AGND** | ANALOG GROUND |
| **I1P** and **I1N** | the metering chip's two current input pins |

## Step 2 — place 10 parts and connect both ends of each

| ✓ | Part | One end goes to | Other end goes to |
|---|---|---|---|
...
| ☐ | **Filter resistor B**, 1.5 kΩ | **AGND** | **I1N** |
| ☐ | **Capacitor**, 33 nF | **I1P** | **I1N** |
| ☐ | **Capacitor**, 10 nF (first) | **I1P** | **AGND** |
| ☐ | **Capacitor**, 10 nF (second) | **I1N** | **AGND** |
| ☐ | Metering chip pin **I1P** | **I1P** | — |
| ☐ | Metering chip pin **I1N** | **I1N** | — |

**That is the whole block.** If every row above is done, Block 4 is finished.
...
Read it as: **everything hangs between CLAMP+ and AGND**, except the two 1.5 kΩ
resistors, which carry the signal onward to the chip.
...
It is essential. The chip compares its two inputs against each other, and if the
two paths have different resistance, noise picked up by the clamp cable stops
cancelling out.

They also correct the clamp's timing error. Making them **bigger than the 1 kΩ
voltage filter resistor in Block 3** is what compensates. 1.5 kΩ is the starting
point — you tune it during calibration, and **you always change both together**.

**3. White wire to CLAMP IN, red wire to AGND.** Backwards, and the meter reports
negative power. Print the colours on the silkscreen next to the screws.
